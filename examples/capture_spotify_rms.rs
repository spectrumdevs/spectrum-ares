...
fn main() {
    ...
    println!(
        "target rationale: using object.serial when available because PipeWire documents `target.object` as accepting object.serial or node.name, and object.serial is the more stable target."
    );
    println!("Press Ctrl+C to stop.");
    println!();

    match pipewire_capture::run_capture_loop(&target, |event| match event {
        ...
        CaptureEvent::Levels {
            ...
            println!(
                "[levels] buffers={} rms={:.5} peak={:.5} frames={} format={}",
                buffers_processed, levels.rms, levels.peak, levels.frames, format
            );
        }
        ...
        Ok(summary) => {
            ...
            if let Some(levels) = summary.last_levels {
                println!(
                    "  last levels: rms={:.5} peak={:.5} frames={}",
